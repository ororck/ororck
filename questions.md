...
2. Pourquoi est-il important de travailler sur une branche dédiée plutôt que directement sur `main` quand on collabore ? travailler sur une branche dédiée permet de ne pas casser main si le code est instable, de faire reviewer le code via une PR avant de merger, et de travailler en parallèle sans bloquer les autres.
3. À quoi ressemble ce workflow dans un vrai projet open source sur GitHub ? forkes le repo, créer une branche sur le fork, pushs, puis ouvrir une Pull Request vers le repo original. Un mainteneur review et merge.
